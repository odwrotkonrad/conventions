...
- CI jobs execute Makefile targets as the main measure to run checks (`make run-repo-ci-precommit-all`, `make test`, `make validate`), not raw commands.
- MR pipelines only (`workflow.rules` on `merge_request_event`).
- One `validate` stage job: `make run-repo-ci-precommit-all` (installs hooks, runs pre-commit over all files), then `git diff --exit-code HEAD`: stale generated docs surface as a diff and fail the job.
